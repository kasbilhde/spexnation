...
function addToCart(newItem) {
    const cart = getCart();
    const existing = cart.items.find(i => i.selectedVariantId === newItem.selectedVariantId);
    if (existing) {
        existing.quantity += 1;        // bump quantity if same variant already in cart
    } else {
        cart.items.push({ ...newItem, cartItemId: crypto.randomUUID() });
    }
    saveCart(cart);
}
