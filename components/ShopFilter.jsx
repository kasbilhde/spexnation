...
const ShopFilter = ({ fopen, setfOpen, selectedBrand, setslectedBrand, selectedGender, setselectedGender, selectedMatarial, setselectedMatarial, selectedFrameType, setselectedFrameType, selectedFrameShape, setselectedFrameShape, selectedPrice, setselectedPrice, selectedLenWidth, setselectedLenWidth, selectedBrideWidth, setselectedBrideWidth, handleClearFilter, filterLength }) => {
    ...
    // handle selected frame type is here
    function handleSeletectedFrameType(e, item) {


        e.preventDefault();


        if (selectedFrameType?.includes(item)) {
            setselectedFrameType(prev => prev.filter((i) => i !== item));
        } else {
            setselectedFrameType(prev => [...prev, item]);
        }
    }
    ...
    // handle selected frame shape is here
    function handleSeletectedFrameShape(e, item) {


        e.preventDefault();


        if (selectedFrameShape?.includes(item)) {
            setselectedFrameShape(prev => prev.filter((i) => i !== item));
        } else {
            setselectedFrameShape(prev => [...prev, item]);
        }
    }
            ...
            <div className="h-full">
                ...
                <div className={`overflow-y-scroll h-[93%] w-full ${fopen ? "block" : "hidden lg:block "}`}>
                    ...
                    <div className="mt-4">
                        ...
                        <div className="ml-1">
                            {
                                frameType?.map((item, index) => {
                                    return (
                                        <div key={index} onClick={(e) => handleSeletectedFrameType(e, item)} className="mt-1 flex items-center gap-2">
                                            <div className="w-fit">
                                                <div className={`flex items-center justify-center text-white cursor-pointer w-4 h-4 ${selectedFrameType?.includes(item) ? "bg-yellow-600" : "bg-transparent border border-gray-300"}`}>
                                                    <TiTick className="text-2xl" />
                                                </div>
                                            </div>
                                            <p className="text-md text-gray-600/70">
                        ...
                        <div className="ml-1">
                            {
                                frameShape?.map((item, index) => {
                                    return (
                                        <div key={index} onClick={(e) => handleSeletectedFrameShape(e, item)} className="mt-1 flex items-center gap-2">
                                            <div className="w-fit">
                                                <div className={`flex items-center justify-center text-white cursor-pointer w-4 h-4 ${selectedFrameShape?.includes(item) ? "bg-yellow-600" : "bg-transparent border border-gray-300"}`}>
                                                    <TiTick className="text-2xl" />
                                                </div>
                                            </div>
                                            <p className="text-md text-gray-600/70">
