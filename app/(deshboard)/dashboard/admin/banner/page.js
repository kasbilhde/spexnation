...
const AccessoriesPage = () => {
    ...
    const fileInputRefForSmall = useRef(null);
    ...
    const handleAccessories = async (e) => {
        ...
        const response = await fetch(`${process.env.NEXT_PUBLIC_BASE_URL}/addbanner`, {
            ...
            body: JSON.stringify({ Route: Route, img: img, smallimg: smallimg, productType: "banner" }),
        });
    ...
    // handle prescription file changes is here
    async function handleImageChangeForSmall(e) {
    ...
    // handle remove function is here
    function handleRemovedImageForSmall(e) {
        e.preventDefault();
        clearFileInput(fileInputRefForSmall);
        setsmallimg('');
    }
                ...
                <div>
                    ...
                    <div className="">
                        <label className="text-md text-gray-600/80 flex flex-col">
                            <span>
                                Upload Hero Banner Image for Large Screens <span className="text-md text-red-600">*</span>
                            </span>
                    ...
                    <div className="mt-4">
                        <label className="text-md text-gray-600/80 flex flex-col">
                            <span>
                                Upload Hero Banner Image for Small Screens <span className="text-md text-red-600">*</span>
                            </span>
                            <span className="text-[10px] bg-yellow-100 text-gray-700 px-1 w-fit">jpg, jpeg, png and webp files are allowed</span>
                            <span className="mt-1 text-[10px] bg-red-200 text-gray-700 px-1 w-fit">Image Must be at least 1900px x 600px and Image Text Max Width Container 1280px</span>
                        </label>
                        <div className="flex items-center gap-2 mt-2 h-full">
                            <input
                                id="file3"
                                ref={fileInputRefForSmall}
                                type="file"
                                accept=".png,.jpg,.jpeg,.webp"
                                onChange={(e) => { handleImageChangeForSmall(e) }}
                                className="hidden w-full border p-2 rounded-md focus:outline-yellow-500/60 text-gray-700/80 cursor-pointer"
                            />
                                ...
                                smallimg ? (
                                    ...
                                    <div>
                                        <div className="relative w-auto h-[60px] md:h-[100px] border border-gray-200 text-gray-500/40 bg-gray-200">
                                            ...
                                            <Image className="w-full h-full object-cover" src={smallimg ? smallimg : defaultImage} width={100} height={1000} alt="prescription" />


                                            <div onClick={(e) => { handleRemovedImageForSmall(e) }} className="absolute top-0 right-0 w-4 h-4 bg-yellow-600 text-white translate-x-1/2 -translate-y-1/2 rounded-full flex items-center justify-center cursor-pointer">
                                                <RxCross2 />
                                            </div>
                                        </div>
                                    </div>
                            ...
                            <tbody className="text-center">
                                {allAccessories?.map((row, index) => (
                                    <tr key={index} className="hover:bg-gray-50">
                                        ...
                                        <td className="p-2 border text-center text-gray-500 flex justify-center items-center">
                                            <div className="bg-gray-100 w-auto h-[80px]">
                                                <Image src={row?.smallimg} alt="accessoriseImage" className="w-full h-full" width="1000" height="1000" />
                                            </div>
                                        </td>
