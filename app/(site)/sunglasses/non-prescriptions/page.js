...
export default function ProductPage() {
    ...
    const fetchSettingsData = async () => {
        ...
        try {
            ...
            const res = await response.json();
            const allBrands = res?.data?.brands?.filter(brand => brand.forProduct === "Non-Prescription Sunglasses");
    ...
    const handleSearchFunction = () => {
        ...
        // frame Type filter
        const ftFilter = mFilter?.filter((item) => {
            if (selectedFrameType?.length > 0) {
                return selectedFrameType?.includes(item?.fType);
            } else {
                return item;
            }
        })
        ...
        // frame shape filter
        const fsFilter = ftFilter?.filter((item) => {
            if (selectedFrameShape?.length > 0) {
                return selectedFrameShape?.includes(item?.fShape);
            } else {
                return item;
            }
        })
        ...
        const pFilter = fsFilter?.filter((item) => {
    ...
    const nonPrescriptionsSunglasses = filteredProducts?.filter((item) => item?.frameType === "Non-Prescription Sunglasses");
